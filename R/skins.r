...
slide.title.bar.ui = function(title, slide.ind, num.slides, am, bi, args) {

  restore.point("slide.title.bar.ui")
  cp =am$opts[["slide.counter.parent"]]
  bdf = am$bdf
  ind.str = NULL
  if (!isTRUE(args[["nonumber"]])) {
    if (is.null(cp)) {
      ...
      prefix = NULL
      # does not work when we preview a single slide
      try({prefix <- bdf$part.prefix[[parent.bi]]},silent = TRUE)
      if (!is.null(prefix)) {
